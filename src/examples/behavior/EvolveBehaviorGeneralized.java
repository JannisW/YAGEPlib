...
public class EvolveBehaviorGeneralized {
	...
	public static final int MAX_CHROMOSOME_HEAD_LENGTH = 44;
	...
	private static GepResult<Boolean> startGeneConfiguration1() {
		...
		re.addGeneticOperator(new Mutation(0.3)); // 0.2
		re.addGeneticOperator(new Inversion(0.2)); // 0.1
	...
	private static GepResult<Boolean> startGeneConfiguration2() {
		...
		potentialTerminals.add(new CheckMemoryTerminal(env));
		potentialTerminals.add(new SetMemoryTerminal(env, true));
		...
		ReproductionEnvironment re = new ReproductionEnvironment();
		re.addGeneticOperator(new Mutation(0.32));
